...
public class EventHandlers {
  ...
  public void OnBanning(BanningEventArgs ev) {
    if (!string.IsNullOrEmpty(_plugin.Config.PublicWebhookUrl) &&
        _plugin.Config.PublicWebhookUrl != _defaultUrl) {
      SendWebhook(ev.Target.Nickname, ev.Target.UserId,
                  ev.Issuer?.Nickname ?? "Server Console", ev.Reason,
                  TimeFormatter(ev.Duration));
    }
    if (!string.IsNullOrEmpty(_plugin.Config.SecurityWebhookUrl) &&
        _plugin.Config.SecurityWebhookUrl != _defaultUrl) {
      SendWebhook(ev.Target.Nickname, ev.Target.UserId,
                  ev.Issuer?.Nickname ?? "Server Console", ev.Reason,
                  TimeFormatter(ev.Duration), false);
    }
  }
  ...
  public void OnKicking(KickingEventArgs ev) {
    if (!string.IsNullOrEmpty(_plugin.Config.PublicWebhookUrl) &&
        _plugin.Config.PublicWebhookUrl != _defaultUrl) {
      SendWebhook(ev.Target.Nickname, ev.Target.UserId,
                  ev.Issuer?.Nickname ?? "Server Console", ev.Reason, "Kick");
    }
    if (!string.IsNullOrEmpty(_plugin.Config.SecurityWebhookUrl) &&
        _plugin.Config.SecurityWebhookUrl != _defaultUrl) {
      SendWebhook(ev.Target.Nickname, ev.Target.UserId,
  ...
  public void OnPlayerOban(BannedEventArgs ev) {
    ...
    if (ev.Details.OriginalName == "Unknown - offline ban") {
      string ticksintime =
          TimeSpan.FromTicks(ev.Details.Expires - ev.Details.IssuanceTime)
      ...
      if (ev.Details.Id.Contains("@steam") &&
          !string.IsNullOrEmpty(_plugin.Config.SteamApiKey)) {
        try {
          GetUserName(ev.Details.Id, out string nickname);

          if (!string.IsNullOrEmpty(_plugin.Config.PublicWebhookUrl) &&
              _plugin.Config.PublicWebhookUrl != _defaultUrl) {
            SendWebhook(nickname, ev.Details.Id,
                        ev.Issuer?.Nickname ?? "Server Console",
                        ev.Details.Reason, time);
          }
          if (!string.IsNullOrEmpty(_plugin.Config.SecurityWebhookUrl) &&
              _plugin.Config.SecurityWebhookUrl != _defaultUrl) {
            SendWebhook(nickname, ev.Details.Id,
                        ev.Issuer?.Nickname ?? "Server Console",
                        ev.Details.Reason, time, false);
          }
        } catch (Exception e) {
          if (!string.IsNullOrEmpty(_plugin.Config.PublicWebhookUrl) &&
              _plugin.Config.PublicWebhookUrl != _defaultUrl) {
            SendWebhook("UNKNOWN (invalid steam api key)", ev.Details.Id,
                        ev.Issuer?.Nickname ?? "Server Console",
                        ev.Details.Reason, time);
          }
          if (!string.IsNullOrEmpty(_plugin.Config.SecurityWebhookUrl) &&
        ...
        if (!string.IsNullOrEmpty(_plugin.Config.PublicWebhookUrl) &&
            _plugin.Config.PublicWebhookUrl != _defaultUrl) {
          SendWebhook("UNKNOWN", ev.Details.Id,
                      ev.Issuer?.Nickname ?? "Server Console",
                      ev.Details.Reason, time);
        }
        if (!string.IsNullOrEmpty(_plugin.Config.SecurityWebhookUrl) &&
  ...
  public void SendWebhook(string bannedPly, string bannedPlyId,
                          string issuerStaffNickname, string reason,
                          string time, bool isPublic = true) {
    ...
    var message =
        new Message {
          ...
          content = _plugin.Config.Content, tts = _plugin.Config.IsTtsEnabled,
          embeds =
              new[] { new DiscordMessageEmbed {
                color = int.Parse(_plugin.Config.HexColor.Replace("#", ""),
                                  NumberStyles.HexNumber),
                author =
                    new DiscordMessageEmbedAuthor {
                      name = name,
                      icon_url = _plugin.Config.ServerImgUrl,
                    },
                title = _plugin.Config.Title, description = desc,
                image =
                    new DiscordMessageEmbedImage {
                      url = _plugin.Config.ImageUrl,
                    },
                footer =
                    new DiscordMessageEmbedFooter {
                      icon_url = _plugin.Config.FooterIconUrl,
                      text = _plugin.Config.FooterTxt,
                    }
              } }
        };
...
public void GetUserName(string userid, out string nickname) {
  var httpWebRequest = (HttpWebRequest)WebRequest.Create(
      $"https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key={_plugin.Config.SteamApiKey}&steamids={userid}");
  ...
  using (var streamReader =
    ...
    nickname = Regex.Match(result, @"\x22personaname\x22:\x22(.+?)\x22")
                   .Groups[1]
                   .Value;
  }
}
